...
class HomeworkListTeacher extends StatefulWidget {
  const HomeworkListTeacher({super.key});

  @override
  State<HomeworkListTeacher> createState() => _HomeworkListTeacherState();
}

class _HomeworkListTeacherState extends State<HomeworkListTeacher> {
  Map screenData = {}; // Last screen data
  List<Map<String, dynamic>> homeworkList = []; // Get homeworks database response
  List<Map<String, dynamic>> studentList = [];  // Get students fatabase response
  late MyUser myUser;
  ...
  void getHomeworks() async {
    homeworkList = await myUser.getHomeworksBySubject(className, subject) ?? [];
  ...
  void getAllStudents() async { // Class information for submissions_list
    studentList = await myUser.getStudentsFromClass(className) ?? [];
  }
  ...
  void didChangeDependencies() {
    ...
    if (!_isDataLoaded) {
      ...
      getAllStudents(); // For submittion list
      getHomeworks();
      _isDataLoaded = true;
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      onTap: () async {
                        deadline = await selectDateTime();
                        dateController.text = _formatDateTime(deadline);
                      },
                    ),
                    SizedBox(height: 10),
                    TextField(
                        ...
                        ElevatedButton(
                          onPressed: () async {
                            ...
                            if (name.isNotEmpty && description.isNotEmpty && datetime.isNotEmpty) {
                              ...
                              bool Status = await myUser.addHomework(
                                  className,
                                  Timestamp.fromDate(deadline),  // From Datetime creates Timestamp
                                  description,
                                  name,
                                  subject);
                              getHomeworks();
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                    content: Text('Something is empty')),
                              );
                            }
                          },
                          child: Text('Save'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: (homeworkList.isEmpty)
          ? const Center(child: Text("No homeworks assigned"))
          : ListView.builder(
        itemCount: homeworkList.length,
        itemBuilder: (context, index) {
          var hw = homeworkList[index];
              ...
              children: [
                Flexible(
                  ...
                  child: Card(
                    ...
                    child: ListTile(
                      title: Text(
                        hw["name"] != null && hw["name"].length > 23
                            ? "${hw["name"].substring(0, 22)}.."
                            : hw["name"] ?? "No Title",
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          Text(
                            hw["description"] != null && hw["description"].length > 68
                                ? "${hw["description"].substring(0, 67)}.."
                                : hw["description"] ?? "No Description",
                          ),
                          const SizedBox(height: 6),
                          Text(hw["teacherShort"],style: const TextStyle(fontSize: 12, color: Colors.grey)),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(
                                "Created: ${_formatTimestamp(hw["createdAt"])}",
                      ...
                      onTap: () {
                        Navigator.pushNamed(
                          context,
                          "home/homeworks/teacher/homework_list/submissions",
                          arguments: {
                            "myUser": myUser,
                            "homework": hw,
                ...
                Flexible(
                  flex: 10,
                    child: (homeworkList[index]["teacherID"]?.id == myUser?.uid)
                        ? OutlinedButton(
                            ...
                            return AlertDialog(
                              ...
                              actions: <Widget>[
                                ...
                                TextButton(
                                  onPressed: () async {
                                    bool status;
                                    status = await myUser?.deleteHomework(homeworkList[index]["uid"]) ?? false;
                                    if (status) {
                                      setState(() {
                                        homeworkList.removeAt(index); // This will remove the item at the current index
                                      });
                                      ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      onPressed: () {
                        print(
                            "smula ${myUser?.shortcut} ${homeworkList[index]["teacherShort"]}");
                      },
                      child: Center(
                        child: const Icon(
                          Icons.delete,
